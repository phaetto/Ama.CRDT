...
public sealed class MinWinsMapStrategyTests
{
    ...
    public void ApplyOperation_Associative_ShouldConverge()
    {
        ...
        var op3 = new CrdtOperation(Guid.NewGuid(), "C", "$.map", OperationType.Upsert, new KeyValuePair<object, object?>("b", 10), timestampProvider.Now());
    ...
    public void Merge_ShouldCombineDataAndResolveConflicts()
    {
        ...
        strategy.ApplyOperation(new ApplyOperationContext(doc2.Data, doc2.Metadata, new CrdtOperation(Guid.NewGuid(), "A", "$.map", OperationType.Upsert, new KeyValuePair<object, object?>("b", 5), timestampProvider.Now()))); // lower value
