...
public sealed class DefaultPartitionSerializationServiceTests
{
    ...
    public async Task WriteHeaderAndReadHeader_ShouldSucceed()
    {
        ...
        // Act
        await service.WriteHeaderAsync(stream, originalHeader, headerSize);
        var readHeader = await service.ReadHeaderAsync(stream, headerSize);
    ...
    public async Task WriteHeader_WhenHeaderTooLarge_ShouldThrow()
    {
        ...
        // Act & Assert
        await Should.ThrowAsync<InvalidOperationException>(() => service.WriteHeaderAsync(stream, header, headerSize));
    }
    ...
    public async Task WriteNodeAndReadNode_LeafNode_ShouldSucceed()
    {
        ...
        // Act
        await service.WriteNodeAsync(stream, originalNode, 0);
        var readNode = await service.ReadNodeAsync(stream, 0);
    ...
    public async Task WriteNodeAndReadNode_AtOffset_ShouldSucceed()
    {
        ...
        // Act
        var bytesWritten = await service.WriteNodeAsync(stream, originalNode, offset);
        var readNode = await service.ReadNodeAsync(stream, offset);
