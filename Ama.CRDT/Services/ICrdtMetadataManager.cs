...
/// <summary>
/// Defines a service for managing CRDT metadata. Responsibilities include initializing or resetting metadata by traversing a document to create LWW timestamps and array positional trackers, pruning old tombstones to control state growth, and advancing version vectors.
/// </summary>
public interface ICrdtMetadataManager
{
    ...
    /// <summary>
    /// Resets an existing <see cref="CrdtMetadata"/> object and initializes it from a document.
    /// This method clears LWW timestamps and positional trackers before recursively traversing the document to repopulate them.
    /// It uses a timestamp generated by the configured <see cref="ICrdtTimestampProvider"/>.
    /// </summary>
    /// <typeparam name="T">The type of the document.</typeparam>
    /// <param name="metadata">The metadata object to reset.</param>
    /// <param name="document">The document object from which to derive metadata.</param>
    void Reset<T>(CrdtMetadata metadata, T document) where T : class;

    /// <summary>
    /// Resets an existing <see cref="CrdtMetadata"/> object and initializes it from a document using a specific timestamp.
    /// This method clears LWW timestamps and positional trackers before recursively traversing the document to repopulate them.
    /// </summary>
    /// <typeparam name="T">The type of the document.</typeparam>
    /// <param name="metadata">The metadata object to reset.</param>
    /// <param name="document">The document object from which to derive metadata.</param>
    /// <param name="timestamp">The timestamp to use for initialization.</param>
    void Reset<T>(CrdtMetadata metadata, T document, ICrdtTimestamp timestamp) where T : class;
