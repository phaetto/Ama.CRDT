...
public sealed class LwwSetTestPoco : IEquatable<LwwSetTestPoco>
{
    ...
    public bool Equals(LwwSetTestPoco? other)
    {
        if (other is null)
        {
            return false;
    ...
    public override bool Equals(object? obj) => Equals(obj as LwwSetTestPoco);
    
    public override int GetHashCode() => Items.Count.GetHashCode();
}

public sealed class LwwSetStrategyProperties
{
    [CrdtProperty]
    public void Idempotence_ApplyingSameOperationTwice_YieldsSameState(long timestamp, string item, bool isRemove)
    {
        ...
        var state1 = new LwwSetTestPoco();
        var meta1 = new CrdtMetadata();
        ApplyOperations(state1, meta1, new[] { op });

        var state2 = new LwwSetTestPoco();
    ...
    private static void ApplyOperations(LwwSetTestPoco state, CrdtMetadata metadata, IEnumerable<CrdtOperation> operations)
    {
        ...
        foreach (var op in operations)
        {
            var context = new ApplyOperationContext(state, metadata, op)
            {
                ...
                FinalSegment = nameof(LwwSetTestPoco.Items)
            };
