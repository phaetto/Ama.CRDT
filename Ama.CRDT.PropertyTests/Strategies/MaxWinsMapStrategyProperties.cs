...
public sealed class MaxWinsMapTestPoco : IEquatable<MaxWinsMapTestPoco>
{
    ...
    public bool Equals(MaxWinsMapTestPoco? other)
    {
        if (other is null)
        {
            return false;
    ...
    public override bool Equals(object? obj) => Equals(obj as MaxWinsMapTestPoco);
...
public sealed class MaxWinsMapStrategyProperties
{
    ...
    public void Idempotence_ApplyingSameOperationTwice_YieldsSameState(long timestamp, string key, int value)
    {
        ...
        var state1 = new MaxWinsMapTestPoco();
        var meta1 = new CrdtMetadata();
        ApplyOperations(state1, meta1, new[] { op });

        var state2 = new MaxWinsMapTestPoco();
    ...
    private static void ApplyOperations(MaxWinsMapTestPoco state, CrdtMetadata metadata, IEnumerable<CrdtOperation> operations)
    {
        ...
        foreach (var op in operations)
        {
            var context = new ApplyOperationContext(state, metadata, op)
            {
                ...
                FinalSegment = nameof(MaxWinsMapTestPoco.Map)
            };
