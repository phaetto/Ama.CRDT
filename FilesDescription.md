...
| `$/Ama.CRDT.UnitTests/Services/Strategies/CausalTreeStrategyTests.cs` | Contains unit tests for the `CausalTreeStrategy`, verifying convergence for concurrent inserts and deletes. |
...
| `$/Ama.CRDT.UnitTests/Services/Strategies/RgaStrategyTests.cs` | Contains unit tests for the `RgaStrategy`, verifying convergence, idempotence, and commutativity for concurrent inserts, deletes, and moves. |
...
| `$/Ama.CRDT/Attributes/CrdtCausalTreeStrategyAttribute.cs` | An attribute to explicitly mark a collection property to use the Causal Tree (CT) strategy for sequence management. |
...
| `$/Ama.CRDT/Attributes/CrdtRgaStrategyAttribute.cs` | An attribute to explicitly mark a collection property to use the RGA (Replicated Growable Array) strategy for stable, concurrent list management. |
...
| `$/Ama.CRDT/Models/CausalAtom.cs` | A class representing a node in a Causal Tree, containing its unique ID, causal link, value, and tombstone status. |
| `$/Ama.CRDT/Models/CausalAtomId.cs` | A record struct for the unique, comparable identifier of a CausalAtom, composed of a timestamp and replica ID. |
...
| `$/Ama.CRDT/Models/RgaIdentifier.cs` | A record struct representing the stable, unique identifier for an item in an RGA (Replicated Growable Array). |
| `$/Ama.CRDT/Models/RgaItem.cs` | A class that encapsulates the metadata for a single item within an RGA, including its stable ID, parent ID, value, and tombstone status. |
...
| `$/Ama.CRDT/Services/Strategies/CausalTreeStrategy.cs` | Implements the Causal Tree (CT) strategy, which models a sequence as a tree of atoms to preserve causal edit history, ideal for collaborative text. |
...
| `$/Ama.CRDT/Services/Strategies/RgaStrategy.cs` | Implements the RGA (Replicated Growable Array) strategy, which uses stable identifiers and causal links to merge concurrent list modifications, including moves. |
